...
async function deployExerciceSolution(deployer,network,accounts)
{
	...
	await Evaluator.submitExercice(myExercice.address,{from:accounts[0]});
	...
	sexCheval=await Evaluator.readSex(myExercice.address,{from:accounts[0]});
	jambesCheval=await Evaluator.readLegs(myExercice.address,{from:accounts[0]});
	wingsCheval=await Evaluator.readWings(myExercice.address,{from:accounts[0]});
	nomCheval=await Evaluator.readName(myExercice.address,{from:accounts[0]});
	...
	//ex7a
	await Evaluator2.submitExercice(myExercice.address,{from:accounts[0]});
	...
	await Evaluator2.ex7c_payForReproduction(6,{from:accounts[0]});
	getBalance= await TDToken.balanceOf(accounts[0]);
	console.log("Ex7b balance" + getBalance.toString());
